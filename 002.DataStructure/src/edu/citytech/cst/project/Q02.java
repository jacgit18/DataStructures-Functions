...
public class Q02 {
	...
	public static void main(String[] args) {
		/* 2.Find the number of employees that worked more 
		 * than 5 hours on a Monday (class: Q02).  */
		List<Employee> list = new EmployeeDAO().findAll();
		
		list.stream().filter(Q02::isHoursGT5)
		.forEach(System.out::println);
		
		long size = list.stream().filter(Q02::isHoursGT5)
				.toArray().length;
